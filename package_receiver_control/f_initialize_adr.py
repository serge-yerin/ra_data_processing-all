...
def f_initialize_adr(serversocket, print_or_not):
    ...
    serversocket.send((b"set prc/dsp/ctl/mdo 1 8192\0"))  # Set FFT size 2048,4096,8192,16384,32768
    data = f_read_adr_meassage(serversocket, print_or_not)
    time.sleep(1)
    serversocket.send((b"set prc/srv/ctl/adr 3 1\0"))  # To apply the ADR DSP parameters (which is set in the [mdo]
    data = f_read_adr_meassage(serversocket, print_or_not)
    ...
    serversocket.send((b"set prc/dsp/ctl/mdo 2 4000\0"))  # Set number of averages spectra in range [16 … 32768]
    data = f_read_adr_meassage(serversocket, print_or_not)
    time.sleep(1)
    serversocket.send((b"set prc/srv/ctl/adr 3 1\0"))  # To apply the ADR DSP parameters (which is set in the [mdo]
    data = f_read_adr_meassage(serversocket, print_or_not)
    ...
    serversocket.send((b"set prc/dsp/ctl/mdo 4 2\0"))  # Width of  frequency band in 1024-steps. WIDTH range [1 … (SFFT-SLINE*1024)/1024]
    data = f_read_adr_meassage(serversocket, print_or_not)
    time.sleep(1)
    serversocket.send((b"set prc/srv/ctl/adr 3 1\0"))  # To apply the ADR DSP parameters (which is set in the [mdo]
    data = f_read_adr_meassage(serversocket, print_or_not)
