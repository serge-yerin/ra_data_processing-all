...
def sources_positions_on_the_sky(start_time, end_time, pulsars_list, sources_list):
    """
    Function crates tracks of sources in the sky for a particular place and time
    """

    current_date = time.strftime("%Y-%m-%d")
    current_time = time.strftime("%H:%M:%S")
    print('\n  Today is ', current_date, ', local time is ', current_time, '\n')
    ...
    print('  Coordinates: \n  * Longitude: ',
          ...
          ' \n  * Latitude:   ' +
          str(utr2_location.lat).replace("d", "\u00b0 ").replace("m", "\' ").replace("s", "\'\' ") + '\n')
    print('  Plot time window: \n  * From: ', str(start_time)[0:19],
    ...
    for i in range(len(pulsars_list)):
        alt, az, dm, p_bar = catalogue_pulsar(pulsars_list[i])
    ...
    fig.text(0.79, 0.04, 'Processed ' + current_date + ' at ' + current_time, 
             fontsize=4, transform=plt.gcf().transFigure)
    fig.text(0.11, 0.04, 'Software version: ' + Software_version + ', yerin.serge@gmail.com, IRA NASU',
             fontsize=4, transform=plt.gcf().transFigure)
    pylab.savefig(newpath + '/' + str(start_time)[0:10] + ' sources elevation.png', bbox_inches='tight', dpi=300)
    # plt.show()

    # Preparing data for correct sky plot (Altitude / Azimuth)
    # Sun
    ...
    for i in range(len(pulsars_list)):
        ax1.plot(pulsars_alt_az[i].az.rad, r_pulsars[i], label=pulsars_list[i], linestyle='--', linewidth=0.5,
                 color=color[i])
    ...
    fig.text(0.210, 0.080, 'Processed ' + current_date + ' at ' + current_time,
             fontsize=4, transform=plt.gcf().transFigure)
    fig.text(0.210, 0.070, 'Software version: ' + Software_version + ', yerin.serge@gmail.com, IRA NASU',
             fontsize=4, transform=plt.gcf().transFigure)
    ...
    if x_rise > 0:
        plt.annotate(str(time_window[x_rise])[11:19], xy=(x_rise, 1 + 0.3), fontsize=6, ha='left')
    if x_set < n_points-1:
        plt.annotate(str(time_window[x_set])[11:19], xy=(x_set, 1 + 0.3), fontsize=6, ha='right')
    ...
    if x_rise > 0:
        plt.annotate(str(time_window[x_rise])[11:19], xy=(x_rise, 2 + 0.3), fontsize=6, ha='left')
    if x_set < n_points-1:
        plt.annotate(str(time_window[x_set])[11:19], xy=(x_set, 2 + 0.3), fontsize=6, ha='right')
    ...
    plt.annotate(str(time_window[int(xmax)])[11:19], xy=(xmax, 2 + 0.3), fontsize = 6, ha='center')
    if len(x_rise) > 0:
        if x_rise[0] > 0: plt.annotate(str(time_window[x_rise[0]])[11:19], xy=(x_rise[0], 2 + 0.3), 
                                       fontsize = 6, ha='left')
    if len(x_set) > 0:
        if x_set[0] < n_points-1: plt.annotate(str(time_window[x_set[0]])[11:19], xy=(x_set[0], 2 + 0.3), 
                                               fontsize = 6, ha='right')
    '''
    ...
    for i in range(len(sources_list)):
        ...
        if x_rise > 0:
            plt.annotate(str(time_window[x_rise])[11:19], xy=(x_rise, n + 0.3), fontsize=6, ha='left')
        if x_set < n_points-1:
            plt.annotate(str(time_window[x_set])[11:19], xy=(x_set, n + 0.3), fontsize=6, ha='right')
...
if __name__ == '__main__':
    ...
    start_time = Time('2022-02-28 00:00:00')
    end_time = Time('2022-03-01 00:00:00')
