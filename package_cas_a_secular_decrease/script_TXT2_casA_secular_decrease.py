...
for source in ['SygA', 'CasA']:
    ...
    for file_no in range(len(file_name_list)):
        ...
        ax1.set_xticklabels(text, fontsize=6, fontweight='bold')
        fig.subplots_adjust(top=0.92)
        pylab.savefig(result_path + '/' + parent_filename + ' Interferometric responce ' + source + ' ' +
                      data_type + ' ' + text_freq + ' MHz.png', bbox_inches='tight', dpi=160)
        plt.close('all')
...
pylab.savefig(result_path + '/' + date_of_experiment_spectra + ' Flux ratio with frequency for files ' +
              file_name_SygA + '_' + file_name_CasA + '.png', bbox_inches='tight', dpi=160)
plt.close('all')
# '''
