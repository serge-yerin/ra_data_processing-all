...
def file_header_jds_read_old(filepath, start_byte, print_or_not):
    """
    !!! Deprecated - used a lot of read operations instead of one !!!
    Reads info from DSP (.jds) data file header and returns needed parameters to main script
    Input parameters:
        filepath - a path to the file to read data
        start_byte - number of byte from which start reading
    ...
    df_creation_time_loc = file.read(32).decode('utf-8').rstrip('\x00')   # Creation time in local time
    df_creation_time_utc = file.read(32).decode('utf-8').rstrip('\x00')   # Creation time in UTC time
