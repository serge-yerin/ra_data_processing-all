...
def file_header_jds_read_old(filepath, start_byte, print_or_not):
    """
    Reads info from DSP (.jds) data file header and returns needed parameters to main script
    Input parameters:
        filepath - a path to the file to read data
    ...
    df_creation_timeLOC = header_bytes[32:64].decode('utf-8').rstrip('\x00')   # Creation time in local time
    df_creation_timeUTC = header_bytes[64:96].decode('utf-8').rstrip('\x00')   # Creation time in UTC time
    ...
    df_obs_place = header_bytes[160:256].decode('utf-8').rstrip('\x00')        # place of observations
