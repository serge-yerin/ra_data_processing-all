...
def pulsar_dm_shift_calculation_aver_pulse(freq_points, fmin, fmax, a_df, a_dt, DM0, pulsar_period):
    """
    ...
    for i in range(freq_points):
        dt = (DM0 / 2.410331) * ((10000./math.pow((fmin + a_df * i), 2)) - (10000./math.pow(fmax, 2)))
        shift_par[i] = -1 * round(pulsar_period * (dt / pulsar_period - math.floor(dt / pulsar_period)) / a_dt)
