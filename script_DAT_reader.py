...
filename = common_path + 'E250714_130330.jds_Data_chA.dat' 
...
AmplitudeReIm = 3 * 10**(-12)  # Colour range of Re and Im dynamic spectra       
...
for j in range(len(typesOfData)):  # Main loop by types of data to analyze
    ...
    if df_filename[-4:] == '.jds':     # If data obrained from DSPZ receiver
        ...
        [df_filename, df_filesize, df_system_name, df_obs_place, df_description,
                ...
                df, frequency, FreqPointsNum] = FileHeaderReaderDSP(filename, 0)
    ...
    for block in range (numOfBlocks):
        ...
        if (typesOfData[j] == 'C_p' or typesOfData[j] == 'C_m' or typesOfData[j] == 'CRe' or typesOfData[j] == 'CIm'): # If analyzing phase/Re/Im - no logarythming needed
